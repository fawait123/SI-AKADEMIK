                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-7 mb-3">
                            ...
                            <div class="input-area relative">
                                ...
                                <input type="date" name="start_date" class="form-control"
                                    value="{{ \Illuminate\Support\Facades\Request::get('end_date') }}"
                                            ...
                                            @foreach ($datas as $item)
                                                <tr>
                                                    ...
                                                    @foreach ($item['detail'] as $absen)
                                                        <td
                                                            class="table-td bg-{{ $absen['absen'] == 'hadir' ? 'success' : '' }}{{ $absen['absen'] == 'ijin' ? 'warning' : '' }}{{ $absen['absen'] == 'alpha' ? 'danger' : '' }}{{ $absen['absen'] == 'sakit' ? 'primary' : '' }}-300 p-2 m-3">
                                                            <span
                                                                class="font-bold">{{ ucfirst($absen['absen'][0]) }}</span>
                                                        </td>
                                                    @endforeach
                                                </tr>
                                            @endforeach
